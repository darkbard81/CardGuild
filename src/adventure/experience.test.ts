...
describe("experience arithmetic", () => {
  it("carries remainders, crosses several levels at once and never touches its input", () => {
    const cases: ReadonlyArray<readonly [CharacterProgressionState, number, CharacterProgressionState, number]> = [
      [{ level: 1, experience: 999, advancements: [] }, 1, { level: 2, experience: 0, advancements: [] }, 1],
      [{ level: 1, experience: 0, advancements: [] }, EXPERIENCE_PER_LEVEL, { level: 2, experience: 0, advancements: [] }, 1],
      [{ level: 1, experience: 999, advancements: [] }, 2, { level: 2, experience: 1, advancements: [] }, 1],
      [{ level: 2, experience: 700, advancements: [] }, 250, { level: 2, experience: 950, advancements: [] }, 0],
      [{ level: 1, experience: 500, advancements: [] }, 4_500, { level: 6, experience: 0, advancements: [] }, 5],
      [{ level: 3, experience: 125, advancements: [] }, 0, { level: 3, experience: 125, advancements: [] }, 0],
    ];
  ...
  it("refuses awards and totals it cannot represent exactly", () => {
    for (const amount of [-1, 0.5, NaN, Infinity, "100" as unknown as number]) {
      expect(() => applyExperience({ level: 1, experience: 0, advancements: [] }, amount)).toThrow();
    }
    // A total past the exact-integer range is refused rather than silently rounded.
    expect(() => applyExperience({ level: 1, experience: 999, advancements: [] }, Number.MAX_SAFE_INTEGER)).toThrow("represented exactly");
    // A malformed progression is refused before any arithmetic runs.
    expect(() => applyExperience({ level: 0, experience: 0, advancements: [] }, 10)).toThrow();
    expect(() => applyExperience({ level: 1, experience: EXPERIENCE_PER_LEVEL, advancements: [] }, 10)).toThrow();
  });
});
...
describe("encounter growth", () => {
  it("pays the authored amount to every seat in order and publishes growth between completion and what comes next", () => {
    ...
    for (const event of gained) {
      ...
      expect(event.previous).toEqual({ level: 1, experience: 0, advancements: [] });
      expect(event.next).toEqual({ level: 1, experience: 200, advancements: [] });
    }
  ...
  it("publishes one LEVEL_UP per level crossed, in seat order after every award", () => {
    const state = withProgression(begun(context, HEROES), { level: 4, experience: 900, advancements: [] });
    ...
    expect(won.state.party.members["party.hero-1"]?.progression).toEqual({ level: 9, experience: 100, advancements: [] });
  });
  ...
  it("pays nothing on defeat, on a reward choice, or for an explicit zero award", () => {
    ...
    expect(fixture.state.party.members["party.hero-1"]?.progression).toEqual({ level: 1, experience: 0, advancements: [] });
  });
  ...
  it("grows parties that started at different levels independently and feeds the next battle", () => {
    ...
    const mixed: AdventureState = {
      ...
      party: {
        members: {
          ...state.party.members,
          "party.hero-1": { ...state.party.members["party.hero-1"]!, progression: { level: 1, experience: 850, advancements: [] } },
          "party.hero-2": { ...state.party.members["party.hero-2"]!, progression: { level: 3, experience: 10, advancements: [] } },
        },
      },
    };
    const won = accept(mixed);
    expect(won.accepted, won.error).toBe(true);
    expect(won.state.party.members["party.hero-1"]?.progression).toEqual({ level: 2, experience: 50, advancements: [] });
    expect(won.state.party.members["party.hero-2"]?.progression).toEqual({ level: 3, experience: 210, advancements: [] });
    expect(won.state.party.members["party.hero-3"]?.progression).toEqual({ level: 1, experience: 200, advancements: [] });
    ...
    const chosen = dispatchAdventureCommand(won.state, { type: "choose-reward", rewardId: offer.rewardId, choiceIndex: 0 }, context);
    expect(chosen.accepted, chosen.error).toBe(true);
    const entered = dispatchAdventureCommand(completeAdvancements(chosen.state, pack), { type: "start-encounter" }, context);
    expect(entered.accepted, entered.error).toBe(true);
