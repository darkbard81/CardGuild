...
export function updateCharacterPicker(root: HTMLElement, actors: readonly Pick<ActorState, "id" | "name" | "definitionId" | "appearanceKey">[], selected: string, catalog: AssetCatalog, choose: (id: string) => void, disabled = false): void {
  root.className = "ui-character-picker";
  root.setAttribute("role", "group"); root.setAttribute("aria-label", "상세 대상");
  const signature = actors.map(actor => `${actor.id}:${actor.appearanceKey ?? actor.definitionId}:${actor.name}`).join("|");
  if (root.dataset.actors !== signature) {
    root.replaceChildren(...actors.map(actor => {
      ...
      const visual = catalog.manifest.actorVisuals[actor.appearanceKey ?? actor.definitionId];
