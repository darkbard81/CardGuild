...
describe("trait vocabulary migration", () => {
  ...
  it("refuses to carry any gameplay change along with the vocabulary change", () => {
    ...
    // The two labels 0.5.0 changed are put back only from exactly what 0.5.0 shipped: a
    // further rename, or a slide back to the old label, is a different pack.
    ...
    expect(TRAIT_VOCABULARY_MIGRATION.verify(relabelled)).toBe(false);
    const reverted = withTrait(PACK, "prone", (trait) => ({ ...trait, name: "Prone Recovery" }));
    expect(TRAIT_VOCABULARY_MIGRATION.verify(reverted)).toBe(false);
