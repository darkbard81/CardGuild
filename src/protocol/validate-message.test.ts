...
describe("protocol v8 structural validation", () => {
  /** v8 carries Character advancement history and member advancement intents. */
  it("rejects legacy and future envelopes outright, whatever its payload", () => {
    ...
    expect(validateClientMessage({ v: 8, ...hello }).ok).toBe(true);
    for (const v of [1, 2, 3, 4, 5, 6, 7, 9, "8", null]) {
    ...
    // Retain the M8 Facing contract while enforcing the new v8 envelope.
    expect(validateClientMessage({
      v: 8, type: "intent", requestId: "v3-end-turn", expectedRevision: 3, intent: { type: "end-turn" },
    }).ok).toBe(false);
    expect(validateClientMessage({
  ...
  it("requires one valid final direction on end-turn, but not on movement targets", () => {
    const message = (intent: unknown) => ({ v: 8, type: "intent", requestId: "facing", expectedRevision: 1, intent });
  ...
  it("accepts hello, party/claim intents, and actor-id-free combat intents", () => {
    ...
    expect(validateClientMessage({
      v: 8, type: "intent", requestId: "resume", expectedRevision: 0, intent: { type: "resume-adventure" },
    }).ok).toBe(true);
    expect(validateClientMessage({
      v: 8, type: "intent", requestId: "resume-injection", expectedRevision: 0,
...
describe("member growth on protocol v8", () => {
  const envelope = (choice: unknown) => ({ v: 8, type: "intent", requestId: "growth", expectedRevision: 4,
