...
새 모험 → 필요한 인증 → 환영 3페이지 → 캐릭터 생성. 명시적 새 모험 선택마다 client-memory의 welcomePending을 초기화한다. 완료·건너뛰기 후 같은 생성 시도의 오류/재로그인에서는 다시 표시하지 않는다. 기존 CreationDraft를 유지하고 생성 폼의 이름 입력으로 포커스를 보낸다. Escape는 시작 화면의 새 모험 버튼으로 돌아간다. 이어하기, Guest 참가, 빈 로비 재접속에는 삽입하지 않는다. 계정·Save·프로토콜에는 필드를 추가하지 않았으며, 대화는 Campaign/캐릭터 생성 요청을 보내지 않는다.

DOM 모달은 전체 화면 입력 레이어와 하단 반투명 패널로 구성한다. 패널은 왼쪽 페이스, 오른쪽 이름/즉시 표시되는 텍스트, 하단 진행 안내와 건너뛰기를 표시한다. 돌아가기·다음·마지막 확정 버튼은 두지 않는다. 화면 어디든 클릭·터치하거나 Enter/Space를 누르면 다음 대사로 진행하며 마지막 페이지에서는 완료한다. 건너뛰기는 전파를 중단해 진행 입력과 겹치지 않는다. 기본 1024×768과 좁은 터치 화면을 지원한다. 포인터를 누른 페이지 revision, 드래그/멀티터치/pointercancel 무시, 마우스 다중 클릭 및 키 반복 차단으로 중복 진행을 막고 독립된 터치 탭은 각각 처리한다. 분기, 자동 진행, 타이핑, 실제 음원/재생, 립싱크는 포함하지 않는다.
