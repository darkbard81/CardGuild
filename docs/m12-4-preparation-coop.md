...
`session/coop.ts` owns the shared preparation, capacity, claim and departure rules. Active ready/between-encounters without Combat and the explicit resume-lobby are safe boundaries. Active reward/result/Combat and pre-creation lobby deny admission/designation/selection. Resume may contain saved Combat; only live delegation changes before Resume, without rebuilding actors, equipment or Combat. Existing claimed credentials may reconnect after departure; new HTTP admissions may not.
...
`CoopPreparationUi` is shared by active preparation and Resume. It shows the protagonist as Host-only and companions with current name, existing standee, level, role, equipment, claimant and connection state. Information opens the existing read-only character sheet. Host sharing does not create a claim. Invitation/copy exposes only the session ID when admission capacity is available, never reconnect credentials.
...
Final local gate passed on 2026-09-21 with `CI=true npm run check && CI=true npm run test:all`:
