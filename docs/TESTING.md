...
커밋하세요. gate가 자산을 조용히 고쳐 놓는 일은 없습니다. 같은 build가 만드는
`art/processed/qc`만은 예외로 tracked하지 않습니다 — 어떤 검사도 그것을 읽지 않고, 사람이
눈으로 볼 때 다시 만들면 되기 때문입니다.
...
**그리고 변화가 실제로 일어났다는 증거를 먼저 요구합니다.** 위 invariant는 전부 standee를
그것이 서 있는 board와 견주므로, **둘 다 아직 안 움직인** 프레임은 스스로와 완벽하게
일치합니다 — 안 변한 sprite가 안 변한 칸 위에, 옛 해상도에서 잰 그 비율로 서 있습니다.
그래서 resize 전 `data-board-corners`를 적어 두고 그것이 달라지는 것을 먼저 본 다음에
invariant를 기다립니다. 제스처도 같습니다: 줌·팬은 corners가 다시 발행된 것을 확인하고
읽습니다.
