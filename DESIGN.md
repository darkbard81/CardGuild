...
- 공통 상세는 `ui-panel ui-panel--dialog ui-character-detail`을 사용한다. `CORE / SKILLS / TRAITS` 탭은 실제 제공되는 데이터로 구성하며, 준비 화면은 현재 HP 대신 최대 HP를 표시한다. 공개된 Character/Creature 데이터만 입력으로 받는다.
