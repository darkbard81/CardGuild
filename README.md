...
전체가 안전영역에 들어오는" 상태이고, 상한은 "`closeUpCells`(3)칸이 안전영역을 채우는"
...
수의 비(`(cols+rows) / (min(cols,3)+min(rows,3))`)가 됩니다 — 그래서 창 크기가 바뀌어도
zoom 범위는 그대로입니다. 회전 때문에 fit은 columns와 rows를 함께 보며, 3x9 같은 세로 맵도
같은 식으로 들어갑니다. 이미 close-up보다 작은 맵은 `minZoomHeadroom`(1.5x)만큼은 항상
확대할 수 있어 휠이 죽지 않습니다. Pan은 보드 중심이 안전영역을 벗어나지 않도록 제한되며, 턴이 시작될 때 해당
