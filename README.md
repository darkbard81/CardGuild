...
방향키도 같은 명령을 보냅니다. 전송 전에는 gameplay state가 변하지 않고, Esc·보드에서
한 칸 이상 벗어난 클릭은 이전 선택으로 돌아가며, 서버가 거절하면 그대로 다시 고를 수
있습니다. Action 0의 자동 선택기 열기와 카드 대상 한 번 터치 실행은 유지합니다. 터치에서
공격 전 Preview를 읽으려면 기존 링 메뉴에서 첫 터치로 행동을 선택합니다.
