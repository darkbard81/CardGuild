...
import { createCampaignAsHost, openApp } from "../support/browser/host-login";
...
test("a host who leaves keeps the campaign, the claim and the revision exactly where they were", async ({ browser }) => {
  // The guest side of this — fallback to host and claim restored on reconnect — is covered
  // above. What is not is the host leaving: they own the campaign, so their absence is the
  // one that could plausibly move something.
  const host = await createPlayer(browser, "Dropping Host");
  const guest = await createPlayer(browser, "Staying Guest");
  try {
    const sessionId = await createHost(host);
    await applyThreeCharacterParty(host.page);
    await joinGuest(guest, sessionId);
    ...
    await expect(guest.page.locator("#app")).toHaveAttribute("data-controlled-actor-ids", "party.hero-2");
    await expect(guest.page.locator("#app")).toHaveAttribute("data-session-hash", hash ?? "");
    ...
    // Ownership is durable, not presence-derived: the guest still cannot list campaigns.
    expect((await guest.page.request.get("/api/campaigns")).status()).toBe(401);

    await host.page.goto("/");
    await expect(host.page.locator("#app")).toHaveAttribute("data-session-status", "connected", { timeout: 20_000 });
    await expect(host.page.locator("#app")).toHaveAttribute("data-viewer-role", "host");
    await expect(host.page.locator("#app")).toHaveAttribute("data-controlled-actor-ids", "party.hero-1,party.hero-3");
    await expect(host.page.locator("#app")).toHaveAttribute("data-session-hash", hash ?? "");
