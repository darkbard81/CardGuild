...
describe("a real WebSocket cooperative session", () => {
  ...
  it("transports nonzero runtime progression and recovers the same gameplay after reconnect", async () => {
    ...
    await accepted(preparing, original, "progression-party", { type: "set-party-composition", actorDefinitionIds: PARTY });
    await accepted(preparing, original, "progression-begin", { type: "begin-adventure" });
    await preparing.close();
    await original.whenIdle();
    // Inject at the existing constructor seam, never through a product command or debug API.
  ...
  it("transports atomic final facing and preserves it in a reconnect snapshot", async () => {
    ...
    const snapshot = await client.snapshot(mark, (message) => message.cause?.requestId === "end-facing");
    ...
    const reconnected = await SocketClient.connect(server.origin, credential);
    sockets.push(reconnected);
    const recovered = await reconnected.snapshot();
  ...
  it("runs a 1P session with the host controlling and editing all three characters", async () => {
    ...
    const client = await SocketClient.connect(server.origin, credential);
    sockets.push(client);
    const initial = await client.snapshot(0, (snapshot) => snapshot.revision === 0);
  ...
  it("falls a guest character back to host without gameplay mutation and restores the claim on reconnect", async () => {
    ...
    await host.whenIdle();
    const fallback = await hostClient.snapshot(
      hostMark,
    ...
    const recoveredBeforeResolve = await beforeResolveReconnect.snapshot(
      0,
    ...
    await host.whenIdle();
    const secondFallback = await hostClient.snapshot(
      secondHostMark,
      (snapshot) => snapshot.cause?.kind === "control" &&
    ...
    const recovered = await reconnect.snapshot(
      0,
      (snapshot) => snapshot.cause?.kind === "resync",
    );
  ...
  it("serializes a 3P claim race, assigns one character each, and converges every client", async () => {
    ...
    await Promise.all([clientB.snapshot(), clientC.snapshot()]);
    ...
    await accepted(hostClient, host, "encounter", { type: "start-encounter" });
    const snapshots = await Promise.all(clients.map((client, index) => client.snapshot(
      marks[index],
  ...
  it("preserves request, credential, payload, and newest-connection transport boundaries", async () => {
    ...
    const replacementSnapshot = await replacement.snapshot(
      0,
      (snapshot) => snapshot.cause?.kind === "resync",
    );
    ...
    const wrongContent = await SocketClient.connect(server.origin, credential, {
      contentIdentity: { ...PRODUCTION_CONTENT.contentIdentity, fingerprint: "fnv1a64:wrong" },
    });
