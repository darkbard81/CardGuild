...
test("shows the victory growth summary on every screen after the battle and keeps the meters readable", async ({ page }, testInfo) => {
  ...
  await openApp(page);
  await page.evaluate(async () => {
    const { AdventureUi } = await import("/src/dom/adventure-ui.ts") as typeof import("../src/dom/adventure-ui");
    const { LoadoutUi } = await import("/src/dom/loadout-ui.ts") as typeof import("../src/dom/loadout-ui");
    const { createAdventureSession } = await import("/src/adventure/runtime.ts") as typeof import("../src/adventure/runtime");
    const { PRODUCTION_CONTENT: content } = await import("/src/content/production-content.ts") as typeof import("../src/content/production-content");
    const { createPresentationCatalog } = await import("/src/presentation/asset-catalog.ts") as typeof import("../src/presentation/asset-catalog");
    const pack = content.pack;
    const initial = createAdventureSession({ definition: content.adventure, actorDefinitions: pack.actorDefinitions, combatContent: pack.combatContent }, {
      members: Object.fromEntries(["hero.aerin", "hero.lyra", "hero.brom"].map((id, index) => {
        const memberId = `party.hero-${index + 1}`;
