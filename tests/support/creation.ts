...
export const creationSource = { ...M7_CONTENT_SOURCE,
  ...
  adventures: M7_CONTENT_SOURCE.adventures.filter(adventure => adventure.id === M7_ADVENTURE_ID).map(adventure => ({ ...adventure,
    encounterIds: adventure.encounterIds.filter(id => id !== "encounter.prone-training"),
    experienceAwards: adventure.experienceAwards.filter(award => award.afterEncounterId !== "encounter.prone-training"),
