...
export const creationSource = { ...M7_CONTENT_SOURCE,
  ...
  adventures: M7_CONTENT_SOURCE.adventures.filter(adventure => adventure.id === M7_ADVENTURE_ID).map(adventure => ({ ...adventure,
    encounterIds: adventure.encounterIds.filter(id => !["encounter.prone-training", "encounter.flanking-training"].includes(id)),
    experienceAwards: adventure.experienceAwards.filter(award => !["encounter.prone-training", "encounter.flanking-training"].includes(award.afterEncounterId)),
    rewards: adventure.rewards.filter(reward => reward.afterEncounterId !== "encounter.prone-training").map((reward, index) => index ? reward : { ...reward, choices: [{ kind: "companion" as const, definitionId: "test.companion-aerin" }] }),
  })), creationPresets: [
