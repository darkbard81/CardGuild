...
import type { CampaignSaveV2 } from "../../../src/server/campaign-save";
...
function totalOwned(save: CampaignSaveV2): number {
...
function highestLevel(save: CampaignSaveV2): number {
...
function lastCommandIsEnemy(save: CampaignSaveV2): boolean {
 ...
 */
function matches(target: FaultTarget, previous: CampaignSaveV2 | null, next: CampaignSaveV2): boolean {
  ...
  campaigns: {
    ...
    commitSave(input) {
      ...
      const previous = lookup.status === "loaded"
        ? JSON.parse(lookup.record.snapshotJson) as CampaignSaveV2
        : null;
      const next = JSON.parse(input.snapshotJson) as CampaignSaveV2;
